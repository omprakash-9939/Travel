...
router.get('/search', optionalAuth, async (req, res) => {
  try {
    ...
    // ── Track hotel search ──────────────────────────────────────────────────
    if (req.user) {
      tracker.trackHotelSearch(req.user._id, {
        city, checkIn, checkOut, guests, rooms, minStars, maxPrice,
        destination: city
    ...
    if (amadeus.isConfigured() && checkIn && checkOut) {
      try {
        const result = await amadeus.searchHotels({ city, checkIn, checkOut, adults: guests, rooms });
        if (result?.hotels?.length) {
          hotels = result.hotels;
          source = 'amadeus';
        }
    ...
    res.json({
      success: true, count: hotels.length, source,
      message: source === 'demo' ? 'Demo hotel data. Add Amadeus keys or run npm run seed.' : undefined,
      hotels
    });
  } catch (error) {
...
router.get('/:id/extras', async (req, res) => {
  try {
    ...
    const attractions = city ? await integrations.googlePlaces.nearbyAttractions(city, map?.lat, map?.lng) : [];
    const restaurants = city ? await integrations.googlePlaces.textSearch(`restaurants in ${city}`, map ? { lat: map.lat, lng: map.lng } : undefined) : [];
...
// GET /api/hotels/:id — with view tracking
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    ...
    // ── Track hotel view ────────────────────────────────────────────────────
    if (req.user) {
      tracker.trackHotelView(req.user._id, {
